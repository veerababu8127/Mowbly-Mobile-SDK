...
- (void) invoke:(MBLJSMessage *)message
{
    ...
    else if ([message.method isEqualToString:@"launchApplication"])
    {
        ...
        if ([[[MBLFeatureBinder defaultBinder] view] isActive])
        {
            ...
            if ([theUrl scheme])
            {
                [MBLApp openExternalUrl:theUrl withTitle:pageName];
            }
            else
            {
                BOOL pageCheck=NO;
                
                //Checkin for pack url or not
                if ([url hasSuffix:@".htm"] || [url hasSuffix:@".html"]) {
                ...
                if (pageCheck) {
                    ...
                    [[[self appDelegate] appViewController] openPageWithName:pageName
                                                                         url:url
                                                                        data:pageData
