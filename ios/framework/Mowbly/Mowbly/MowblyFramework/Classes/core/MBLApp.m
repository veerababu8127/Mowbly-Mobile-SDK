...
+ (void)openExternalUrl:(NSURL *)url {
    ...
    MBLAppDelegate *appDelegate = (MBLAppDelegate *)[[UIApplication sharedApplication] delegate];
    UINavigationController *navigationController = [[appDelegate appViewController] navigationController];

    MBLExternalPageViewController *epvc = [[MBLExternalPageViewController alloc] initWithUrl:url.absoluteString];
...
+ (NSString *)getTranslations {
    ...
    if (! translations) {
        ...
        NSString *langCode = [self getLanguage];
        
        NSError *error = nil;
        translations = [NSString stringWithContentsOfFile:[[MBLApp appDirectory] stringByAppendingFormat:@"/system/i18N/%@.%@", langCode, @"i18N"] 
                                                 encoding:NSUTF8StringEncoding 
                                                    error:&error];
        if (error) {

            LogError(@"Error loading translations. %@", [error localizedDescription]);
        }
    }
