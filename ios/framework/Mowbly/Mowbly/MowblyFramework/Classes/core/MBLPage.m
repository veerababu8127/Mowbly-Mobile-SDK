...
- (NSDictionary *)getViewContext {
    ...
    NSString *translationStr = [MBLApp getTranslations];
    if (translationStr) {

        NSDictionary *translations = [[CJSONDeserializer deserializer] deserializeAsDictionary:[[translationStr stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding] dataUsingEncoding:NSUTF8StringEncoding] error:&error];
        [ctx setValue:translations forKey:TRANSLATIONS];        
    }
